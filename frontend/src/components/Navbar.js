  ...
  <div className="flex items-center gap-2">
    <img src="/logo.png" alt="Logo" className="w-auto h-24" />
          ...
          <div className="flex-1 max-w-xl mx-auto flex items-center gap-2">
            ...
            {/* Menu Button */}
            <button onClick={()=> { filterOpen === true ? setFilterOpen(false): setFilterOpen(true)}} className="p-3 rounded-xl bg-main text-white hover:bg-opacity-90">
              <ListFilter className="w-8 h-5" />
            </button>
            <div className=" relative">
            {
              filterOpen === true ? <div className={` absolute gap-2 bg-white mt-6 z-[2000] p-4  rounded-xl left-[-180px]  border w-72 `}>
                <h3 className="mb-2">Tags</h3>
                <div className={` flex gap-2 flex-wrap`}>
                {
                  tags.length > 0 ? tags.map((tag)=> {
                    return(
                      <div key={tag._id} className="" onClick={()=>navigate(`/article/filter?filter=${tag._id}`)}>
                        <div className=" bg-main rounded-xl cursor-pointer text-white py-2 px-3">{tag.name} </div>
                      </div>
                    )
                  }) : <div> no tages</div>
                }
